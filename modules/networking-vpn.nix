...
# ---------------------------------------------------------------------------
  ...
  systemd.services.vpn-namespace = {
    ...
    script = ''
      ...
      # 4. Route local veth link & host LAN traffic back through veth-mullvad
      ip netns exec mullvad ip route add 10.200.1.0/24 dev veth-mullvad || true
      ip netns exec mullvad ip route add ${hostSubnet} via 10.200.1.1 dev veth-mullvad || true
