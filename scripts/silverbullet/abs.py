...
def make_reading_log_template(year):
    ...
    return (
        ...
        "| Books | Comics |\n"
        "|:---:|:---:|\n"
        "| ${#query[[from p = index.pages(\"book\") where p.year == " + y + "]]} | "
        "${#query[[from p = index.pages(\"comic\") where p.year == " + y + "]]} |\n\n"
        "**Sort by:** "
        "${widgets.commandButton(\"Rating\", \"ReadingLog: Sort\", {\"rating\"})} "
        "${widgets.commandButton(\"Name\", \"ReadingLog: Sort\", {\"name\"})} "
        "${widgets.commandButton(\"\U0001f4c5 Date finished\", \"ReadingLog: Sort\", {\"date\"})}\n\n"
        "## \U0001f4da Books\n\n"
        "${query[[\n"
        "  from p = index.pages(\"book\")\n"
        "  where p.year == " + y + "\n"
        "  order by (p.rating or 0) desc, p.name asc\n"
        "  select rating_line(p, \"\U0001f4d6\")\n"
        "]]}\n\n"
        "## \U0001f4a5 Comics\n\n"
        "${query[[\n"
        ...
        "  where p.year == " + y + "\n"
        "  order by (p.rating or 0) desc, p.name asc\n"
        "  select rating_line(p, \"\U0001f4a5\")\n"
        "]]}\n"
    )
